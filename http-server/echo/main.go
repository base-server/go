...
func setHandler(server *echo.Server) {
	server.RegisterHandler(http.MethodGet, "/v1/test/:id", echo.WrapHandlerFunc(handler.Get))
	server.RegisterHandler(http.MethodPost, "/v1/test", echo.WrapHandlerFunc(handler.Post))
	server.RegisterHandler(http.MethodDelete, "/v1/test/:id", echo.WrapHandlerFunc(handler.Delete))
}
...
func main() {
	...
	start := func(log *slog.Log) error {
		...
		return server.Start(config.Get("http.echo.address").(string), listenAndServeFailureFunc)
	}
